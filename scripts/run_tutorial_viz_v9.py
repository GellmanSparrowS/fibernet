#!/usr/bin/env python3
"""
FiberNet v4 Tutorial — Visualization Generator v9

Fixes from v8:
- 01/02: 12 unit types gallery (3×4 grid) instead of 20 voronoi
- 04: 1 structure 8-frame trajectory (2×4 grid)
- 05: Stress distribution with better relaxation
- 09: Real RL training with ParametricStructureEnv
- Force scaling: stiffness=1e3 for tens of Newtons (not hundreds of kN)
...
STIFFNESS = 1e3  # Reduced from 1e5 for reasonable forces (tens of N)
BOX_SIZE = (1.0, 1.0)
GRID = (2, 2)
TARGET_STRETCH = 1.5
DAMPING = 0.5
...
print("="*70)
print("FiberNet v4 Tutorial — Visualization Generator v9")
print("="*70)
...
# ═══════════════════════════════════════════════════════════════
# 02: Gallery of 12 unit types (deformed with point_displacements)
# ═══════════════════════════════════════════════════════════════
...
for unit_name in UNITS:
    try:
        # Try with n_pts_per_side=2 and auto-generated displacements
        g = pattern_2d(unit=unit_name, box=BOX_SIZE, grid=GRID, 
...
# ═══════════════════════════════════════════════════════════════
# Phase 5: Simulation (with checkpoint)
# ═══════════════════════════════════════════════════════════════
print("\nPhase 5: Running simulations (checkpoint enabled)")
print("-"*70)

engine = TaichiEngine()
results = []

for i, g in enumerate(tqdm(structures_voronoi, desc="Simulating")):
...
for theme_name in THEMES_LIST:
    ...
    fig.suptitle('Stretch Simulation Trajectory (Voronoi 0)', 
                 color=colors['text'], fontsize=14, fontweight='bold', y=0.98)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
...
for theme_name in THEMES_LIST:
    ...
    fig.suptitle('Stress Distribution on Edges (Voronoi 0)', 
                 color=colors['text'], fontsize=14, fontweight='bold', y=0.98)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
...
for theme_name in THEMES_LIST:
    ...
    fig.suptitle('Batch Simulation Statistics (20 Voronoi)', 
                 color=colors['text'], fontsize=14, fontweight='bold', y=0.98)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    save_fig(fig, '07_batch_stats', theme_name, colors)

# ═══════════════════════════════════════════════════════════════
# 08: Force-feature importance
# ═══════════════════════════════════════════════════════════════
...
# Run real RL training
env = create_rl_environment(
    unit='voronoi', grid=(2,2), n_pts_per_side=3,
    stiffness=STIFFNESS, num_steps=500  # Reduced for speed
)
...
for ep in tqdm(range(n_episodes), desc="RL training"):
    ...
    # Simple random policy with exploration
