...
from fslab.surface_mapping import load_obj,subdivide_quads
...
class Demo:
    ...
    def generate(self,unit,grid,amplitude,seed):
        if unit not in all_unit_keys() or not 1<=int(grid)<=12 or not 0<=float(amplitude)<=1:raise gr.Error('参数超出在线演示范围')
    ...
    def simulate(self,state,stretch,weld):
        ...
        if not 1.05<=float(stretch)<=5:raise gr.Error('拉伸比须在 1.05–5 之间')
    ...
    def train(self,state,count,model,acquisition="random",parameters="{}",progress=gr.Progress()):
        ...
        if not 12<=count<=1000 or model not in MODEL_SPECS:raise gr.Error('请检查样本数量或模型')
    ...
    def inverse(self,state,budget,target,progress=gr.Progress()):
        ...
        if not 4<=budget<=1000 or target not in ('J','C','linear'):raise gr.Error('请检查搜索预算或目标')
    ...
    def manufacture(self,state,source,diameter,extent,progress=gr.Progress()):
        ...
        if not .5<=float(diameter)<=8 or not 60<=float(extent)<=250:raise gr.Error('请检查制造尺寸')
        network=state['network'] if curved else compile_planar(factory)
        if len(network.edges)>120000:raise gr.Error('在线资源有限，请减少网络数量；完整规模可使用桌面版')
...
class StudioDemo(Demo):
    def map_obj(self,state,model,upload,density,points):
        ...
        if not 12<=int(density)<=10000 or not 2<=int(points)<=10:raise gr.Error('网格参数超出在线范围')
        ...
        while len(faces)*4<=int(density):
            vertices,faces=subdivide_quads(vertices,faces,levels=1,max_faces=int(density))
        network=compile_surface(vertices,faces,factory,overscale=1.06)
        axis='y' if upload or model!='金字塔' else 'z'
        return dict(state,network=network,up_axis=axis,surface_faces=len(faces)),surface_figure(network,axis),f'{len(faces)} 个曲面网格 · {len(network.edges):,} 条纤维边 · 欧拉闭合路径'
...
def create_app():
    ...
    with gr.Blocks(title='FiberScope 3.0 · 纤维网络材料设计',theme=gr.themes.Soft(primary_hue='blue'),delete_cache=(3600,86400),css=".gradio-container {max-width:1440px!important} #hero {background:linear-gradient(120deg,#101e35,#174c68);padding:32px;border-radius:20px;margin-bottom:20px} #hero h1,#hero h3,#hero p {color:white!important} .tab-nav button {padding:14px!important} footer {display:none!important}") as app:
        ...
        gr.Markdown('### 线上展示 Demo · 桌面版为完整版本\n此页面用于体验核心流程。完整功能与大型任务，请下载桌面版。\n\n[GitHub 开源代码](https://github.com/GellmanSparrowS/fibernet) · [下载 Windows 完整版](https://github.com/GellmanSparrowS/fibernet/releases/tag/fiberscope-v3.0.0)')
        state=gr.State({},time_to_live=14400,delete_callback=cleanup_session)
        with gr.Tab('1 · 结构生成'):
            with gr.Row():
                unit=gr.Dropdown([(unit_display(k,'zh'),k) for k in all_unit_keys()],value='square',label='基本单元')
                grid=gr.Slider(1,12,value=3,step=1,label='网络数量（每方向）')
        ...
        with gr.Tab('2 · 拉伸仿真'):
            stretch=gr.Slider(1.05,5,value=2,step=.05,label='目标拉伸比');weld=gr.Checkbox(True,label='焊接锚点')
        ...
        with gr.Tab('4 · 机器学习'):
            with gr.Row():
                count=gr.Slider(12,1000,value=120,step=1,label='物理标注样本数量')
        ...
        with gr.Tab('5 · AI 逆向设计'):
            with gr.Row():target=gr.Dropdown(['J','C','linear'],value='J',label='目标曲线');budget=gr.Slider(4,1000,value=60,step=1,label='物理评估预算')
        ...
        with gr.Tab('6 · 三维曲面'):
            surface_model=gr.Dropdown(list(SURFACE_MODELS),value='金字塔',label='桌面版曲面模型')
            with gr.Row():
                upload=gr.File(file_types=['.obj'],type='filepath',label='导入 OBJ（三角面 / 四边面）')
                density=gr.Slider(12,10000,value=1000,step=1,label='目标曲面网格数')
                surface_points=gr.Slider(2,10,value=5,step=1,label='每段采样点数')
        ...
        with gr.Tab('7 · 制造导出'):
            with gr.Row():
                ...
                diameter=gr.Slider(.5,8,value=2,step=.1,label='纤维直径 / mm')
                extent=gr.Slider(60,250,value=250,step=10,label='打印空间 / mm')
            ...
            gr.Markdown('点击生成后，如尚未显示三维模型且任务未报错，说明魔搭社区集群仍在排队或计算，请稍候。曲面结构通常比平面结构耗时更长。')
