#!/usr/bin/env python3
"""
FiberNet FEM Scenarios — 经典力学模拟场景可视化

生成 Taichi/FEM 经典使用场景的可视化，包括：
1. 单轴拉伸 (弹性)
2. 单轴拉伸 (断裂)
3. 双轴拉伸
4. 单轴压缩
...
def scenario_01_uniaxial_tension(state, skip):
    ...
    result = fem.uniaxial_tension(strain=0.02, deformation_scale=15)
    ...
    fig = render_deformation(
        ...
        line_width=1.5,
        title=f"Uniaxial Tension (ε=0.02) — E*={result.effective_youngs_modulus:.2e} Pa, ν*={result.effective_poissons_ratio:.3f}"
    )
...
def scenario_02_uniaxial_fracture(state, skip):
    ...
    g = pattern_2d(unit="reentrant", box=(10, 10), grid=(4, 4),
                   n_pts_per_side=4, seed=42, unit_kwargs={"angle": 25})
    fem = BeamFEM(g, default_E=1e9, default_nu=0.3)

    # Progressive fracture
    result = fem.tensile_fracture(
        max_strain=0.05, n_steps=10,
        strength=5e7, deformation_scale=20
    )

    # Save
    fem.save_result(result, str(OUTPUT_DIR / "uniaxial_fracture.json"))

    # Plot: final state + history
    ...
    if result.deformed_graph:
        render_graph(result.deformed_graph, ax=ax1, theme="dark",
                     color_by="displacement",
                     colormap="inferno", line_width=1.5, show_nodes=False,
                     title="Final State (Fracture)")
    ...
    fig.suptitle("Progressive Fracture Under Tension",
...
def scenario_03_biaxial(state, skip):
    ...
    g = pattern_2d(unit="kagome", box=(10, 10), grid=(4, 4),
                   n_pts_per_side=4, seed=42)
    fem = BeamFEM(g, default_E=1e9, default_nu=0.3)
    result = fem.biaxial_tension(strain_x=0.015, strain_y=0.015, deformation_scale=15)
    ...
    fig = render_deformation(
        ...
        color_by="displacement",
        line_width=1.5,
        title=f"Biaxial Tension (εx=0.015, εy=0.015) — E*={result.effective_youngs_modulus:.2e} Pa"
    )
    save_fig(fig, name, state)
...
def scenario_04_compression(state, skip):
    ...
    g = pattern_2d(unit="honeycomb", box=(10, 10), grid=(5, 5),
                   n_pts_per_side=4, seed=42)
    fem = BeamFEM(g, default_E=1e9, default_nu=0.3)
    result = fem.compression(strain=0.02, deformation_scale=15)
    ...
    fig = render_deformation(
        ...
        color_by="displacement",
        line_width=1.5,
        title=f"Compression (ε=-0.02) — E*={abs(result.effective_youngs_modulus):.2e} Pa"
    )
    save_fig(fig, name, state)
...
def scenario_05_shear(state, skip):
    ...
    g = pattern_2d(unit="chiral", box=(10, 10), grid=(4, 4),
                   n_pts_per_side=4, seed=42)
    fem = BeamFEM(g, default_E=1e9, default_nu=0.3)
    result = fem.shear_test(strain=0.02, deformation_scale=15)
    ...
    fig = render_deformation(
        ...
        color_by="displacement",
        line_width=1.5,
        title=f"Shear Test (γ=0.02) — G*={result.effective_youngs_modulus:.2e} Pa"
    )
    save_fig(fig, name, state)
...
def scenario_06_stress_strain_curve(state, skip):
    ...
    for (unit, kwargs), color in zip(structures, colors):
        g = pattern_2d(unit=unit, box=(10, 10), grid=(4, 4),
                       n_pts_per_side=3, seed=42, unit_kwargs=kwargs)
        fem = BeamFEM(g, default_E=1e9, default_nu=0.3)
    ...
    ax.set_title("Stress-Strain Curves: Multi-Structure Comparison",
...
def main():
    ...
    print("=" * 60)
    print("FiberNet FEM Scenarios — Classic Simulation Visualizations")
    print("=" * 60)
