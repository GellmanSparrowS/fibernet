...
from fslab.structure import StructureFactory, UNIT_PRESETS
...
def spec_text(f: StructureFactory) -> str:
    return (f"{f.unit} · {f.grid_x}×{f.grid_y} · pts {f.n_pts_per_side} "
            f"· seed {f.seed}")
...
class LineEditor(QWidget):
    ...
    def _geom(self):
        ...
        L = max(140.0, min(w - 2 * m, (h - 16) / 0.6))
        x0 = (w - L) / 2.0
    ...
    def _move(self, p):
        ...
        dx = (p.x() - (x0 + t * L)) / L
        dy = (p.y() - y) / L
        self.disp[k] = [float(max(-0.35, min(0.35, dx))),
                        float(max(-0.35, min(0.35, dy)))]
        self.update()
        self.changed.emit()
...
class StructureTab(QWidget):
    ...
    def _build(self):
        ...
        self.unit_combo.addItems(list(UNIT_PRESETS.keys()))
        self.unit_combo.setCurrentText("reentrant")
        ...
        g.addWidget(self.lbl_grid, 1, 0); g.addWidget(self.grid_x, 1, 1)
        g.addWidget(self.lbl_x, 1, 2, Qt.AlignCenter)
        g.addWidget(self.grid_y, 1, 3)
        g.addWidget(self.lbl_pts, 2, 0); g.addWidget(self.pts, 2, 1, 1, 3)
        g.addWidget(self.lbl_pert, 3, 0); g.addWidget(self.pert, 3, 1, 1, 3)
        g.addWidget(self.lbl_seed, 4, 0); g.addWidget(self.seed, 4, 1, 1, 3)
        g.addWidget(self.dice_btn, 5, 0, 1, 4)
    ...
    def load_spec(self, f: StructureFactory):
        ...
        self.unit_combo.setCurrentText(f.unit)
    ...
    def _refresh_gallery(self):
        for i in range(self.gallery.count()):
            ...
            try:
                f = StructureFactory(
                    unit=it.text(), grid_x=1, grid_y=1,
                    n_pts_per_side=self.pts.value(),
                    perturbation=self.pert.value() / 100.0,
                    seed=self.seed.value(),
                    line_displacements=self.editor.displacements()).clamped()
    ...
    def _update_preview(self):
        try:
            f = StructureFactory(
                unit=self.unit_combo.currentText(), grid_x=1, grid_y=1,
                n_pts_per_side=self.pts.value(),
                perturbation=self.pert.value() / 100.0,
                seed=self.seed.value(),
                line_displacements=self.editor.displacements()).clamped()
            ...
            pos = np.asarray(g.node_positions(), float)[:, :2]
            edges = np.asarray(g.edge_array(), int)[:, :2]
            pm = self._draw_network(pos, edges, 260, 200, line_w=1.4)
    ...
    def _do_export_json(self):
        path, _ = QFileDialog.getSaveFileName(
            ...
            f"fiberscope_{self.unit_combo.currentText()}.json",
            "JSON (*.json)")
        if not path:
            return
        try:
    ...
    def _do_export_svg(self):
        path, _ = QFileDialog.getSaveFileName(
            ...
            f"fiberscope_{self.unit_combo.currentText()}.svg",
            "SVG (*.svg)")
        if not path:
            return
        try:
