"""Resumable real-asset manufacturing matrix: python scripts/validate_manufacturing25.py."""
...
class ManufacturingValidation:
    def run(self):
        ...
        report=root/'docs/validation/manufacturing25.json'
        sources=['fslab/manufacturing.py','fslab/print_export.py','fslab/structure.py','fslab/surface_mapping.py']
        ...
        for name in ('3_Lung_quad_500.obj','4_Heart_quad_500.obj','6_vans_500.obj'):
            ...
            for unit in all_unit_keys():
                ...
                print('[manufacturing25]',key,'PASS',flush=True)
            key=name+':solid'
            if not results.get(key,{}).get('pass'):
                ...
                atomic_json(report,dict(source_hash=signature,results=results))
                print('[manufacturing25]',key,'PASS',flush=True)
        print('[manufacturing25]',len(results),'complete cases')
