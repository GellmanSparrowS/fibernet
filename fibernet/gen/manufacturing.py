"""Canonical planar multigraph compilation and continuous Euler routes."""
...
@dataclass
class PlanarManufacturingConfig:
    """Independent library configuration for the APP's planar topology model.

    Coordinates use the input unit cell scale of 10. This is a graph route
    constraint, not a complete physical printability certificate.
    """
    unit: str = 'square'
    grid_x: int = 3
    grid_y: int = 3
    ...
    def effective_spectrum(self):
        points = self.n_pts_per_side
        spectrum = np.asarray(fit_spectrum(self.line_displacements, points),
                              dtype=float).reshape(-1, 2)
        if self.spectrum_resolved:
            return spectrum
        rng = np.random.default_rng(self.seed)
        if self.line_displacements is None and not np.any(spectrum) and self.perturbation and not self.wave:
            return np.round(rng.uniform(-.25, .25, spectrum.shape) * self.perturbation, 3)
        if self.wave and points:
            spectrum = rng.uniform(-.08, .08, spectrum.shape)
        if self.perturbation:
            spectrum *= 1. + rng.uniform(-self.perturbation, self.perturbation,
                                         spectrum.shape)
        return spectrum

    def build(self):
        """Provide a zero-control-point cell for the shared compiler."""
        if self.topology != 'legacy':
            raise ValueError('build() is only used for the legacy base cell')
        ...
        if self.unit == 'square':
            return pattern_2d(points=[(0., 0.), (CELL, 0.),
                                      (CELL, CELL), (0., CELL)],
                              closed=True, grid=(1, 1), box=(CELL, CELL),
                              radius=self.radius, seed=self.seed)
        spec = _CELL_SPECS.get(self.unit)
        if spec is not None:
            graph = StructureGraph(dimension=2, box_size=[CELL, CELL])
            for x, y in spec['nodes']:
                graph.add_node([x * CELL, y * CELL])
            for a, b in spec['edges']:
                graph.add_edge(int(a), int(b), radius=self.radius)
            return graph
        return pattern_2d(unit=self.unit, grid=(1, 1), box=(CELL, CELL),
                          radius=self.radius, seed=self.seed)


_CELL_SPECS = {
    'ring': {
        'nodes': [(0.5 + 0.36 * np.cos(k * np.pi / 2),
                   0.5 + 0.36 * np.sin(k * np.pi / 2)) for k in range(4)]
                 + [(1., .5), (.5, 1.), (0., .5), (.5, 0.)],
    ...
    'kagome': {
        'nodes': [(0., 0.), (1., 0.), (1., 1.), (0., 1.),
                  (.5, 0.), (1., .5), (.5, 1.), (0., .5)],
        'edges': [(0, 4), (4, 1), (1, 5), (5, 2), (2, 6), (6, 3),
                  (3, 7), (7, 0), (4, 5), (5, 6), (6, 7), (7, 4)],
    },
    'octagon': {
        'nodes': [(1 / (2 + np.sqrt(2)), 0.),
                  (1 - 1 / (2 + np.sqrt(2)), 0.),
                  (1., 1 / (2 + np.sqrt(2))),
                  (1., 1 - 1 / (2 + np.sqrt(2))),
                  (1 - 1 / (2 + np.sqrt(2)), 1.),
                  ...
                  (0., 1 - 1 / (2 + np.sqrt(2))),
                  (0., 1 / (2 + np.sqrt(2)))],
        'edges': [(k, (k + 1) % 8) for k in range(8)],
    },
}
...
def manufacturable_graph(config):
    """Compile a planar route and preserve parallel fiber identities in a graph."""
    from fibernet.core.structure_graph import StructureGraph
    network = compile_planar(config)
    if len(network.positions) > MAX_NODES:
        raise MemoryError('structure exceeds graph node budget')
    graph = StructureGraph(dimension=2)
    for point in network.positions:
        graph.add_node(point, merge=False)
    for a, b in network.edges:
        graph.add_edge(int(a), int(b), radius=config.radius,
                       allow_parallel=True)
    graph.metadata.update(topology='topnet26', topology_id=network.topology_id,
...
def compile_surface(vertices, faces, factory, overscale=1.06):
    ...
    from .surface_geometry import bilinear, coarsen_quads
    vertices = np.asarray(vertices, dtype=float)
    raw_faces = np.asarray(faces)
    if (vertices.ndim != 2 or vertices.shape[1] != 3
            or not np.isfinite(vertices).all()
            or raw_faces.ndim != 2 or raw_faces.shape[1] != 4
            or not np.issubdtype(raw_faces.dtype, np.integer)
            or len(raw_faces) == 0 or not 1. <= overscale <= 1.3):
        raise ValueError('surface requires finite 3D vertices, quad indices and overscale 1..1.3')
    faces = raw_faces.astype(np.int64, copy=False)
    if (faces.min() < 0 or faces.max() >= len(vertices)
            or np.any(np.diff(np.sort(faces, axis=1), axis=1) == 0)):
        raise ValueError('surface has invalid or repeated face indices')
