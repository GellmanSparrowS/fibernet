...
'''
import numpy as np
from .width_contact import ContactConfig, candidate_pairs
...
class SnapshotFeatureExtractor:
    """Bounded structural, planar-pore and optional finite-width snapshot analysis.

    Coordinates and width share the caller's length unit. Pore areas and
    contact overlap areas use its square. Parallel edges remain independent.
    """

    def __init__(self, include_contact=False, contact_config=None,
                 max_nodes=5000, max_edges=800):
        ...
        if self.max_nodes < 1 or self.max_edges < 1:
            raise ValueError("feature graph budgets must be positive")

    def extract(self, positions, edges, rect=None):
        ...
        if raw_edges.size == 0:
            raw_edges = np.empty((0, 2), dtype=int)
        if (raw_edges.ndim != 2 or raw_edges.shape[1] != 2 or
                not np.issubdtype(raw_edges.dtype, np.integer)):
            raise ValueError("edges must be an integer (E, 2) array")
        edges = raw_edges.astype(int, copy=False)
        if (len(positions) > self.max_nodes or len(edges) > self.max_edges):
            raise MemoryError("snapshot feature graph exceeds configured budget")
        if (len(edges) and (edges.min() < 0 or edges.max() >= len(positions) or
                            np.any(edges[:, 0] == edges[:, 1]))):
            raise ValueError("edges contain invalid node indices or self-loops")
        if rect is not None:
            region = np.asarray(rect, dtype=float)
            if region.shape != (4,) or not np.isfinite(region).all():
                raise ValueError("rect must contain four finite bounds")
        return compute_features(positions, edges, rect=rect,
                                contact_config=self.contact_config,
                                include_contact=self.include_contact)

    def extract_graph(self, graph, rect=None):
        """Analyze a graph exposing node_positions() and edge_array()."""
        positions = np.asarray(graph.node_positions(), dtype=float)
        if positions.ndim == 2 and positions.shape[1] == 3:
            if len(positions) and np.ptp(positions[:, 2]) > 1e-9:
                raise ValueError("snapshot features require a planar graph")
            positions = positions[:, :2]
        return self.extract(positions, graph.edge_array(), rect=rect)
