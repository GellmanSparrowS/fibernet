...
本文件供后续 Work 模式拼图、复跑、整理引用和撰文。它是任务书和证据索引，不是投稿手稿。当前代码属于原有 `fibernet` 仓库的 4.2.0 开发分支；FiberScope 为同仓库 APP。Python 3.10 库测试 368 通过、19 跳过，APP 测试 25/25；本地 wheel 隔离安装及冻结 APP 的 Windows 可携带性门槛通过。没有跨机器、Linux/macOS、材料实验或打印后性能验证。研究数据与图不得超出这些条件解释。
...
| 可由 Python 独立运行 | 4.2.0 wheel 在 Python 3.10 隔离目录实跑自定义单元 140 边闭合路线、平行梁反力比 2、快照特征，未导入 APP | 不等于跨操作系统安装验收 |
